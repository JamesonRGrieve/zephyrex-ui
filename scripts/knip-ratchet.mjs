 ...
 * its standard categories. Per-category counts cannot rise. When a category's
 * count reaches 0 it auto-flips to "strict mode" — further occurrences in that
 * category are a hard fail with no `--update` escape.
 *
...
if (failures.length) {
  ...
  console.error('Run `pnpm knip` to see what changed. Either fix or run: pnpm knip:ratchet:update');
  process.exit(1);
}
...
if (improved && !newlyStrict.length) {
  console.log('[knip-ratchet] OK: counts decreased. Lower baseline: pnpm knip:ratchet:update');
